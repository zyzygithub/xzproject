...
<p hidden="hidden" id="upanddown">待审核</p>
...
<input type="text" class="form-control form-group" id="search" placeholder="请输入关键字 自动检索 " style="width:200px;"><!-- <strong class="form-group">搜索：</strong> --> 
		 ...
		 <table id="tablegood" class="table"  style="width:100%">
							<thead>
								<tr>
									...
									<th></th><th></th><th></th><th></th><th>
										<input type="button" class="downgoods alert-info " value="取消审核" />
							...
							<t:forEach items="${requestScope.pd.data}" var="ggt" begin="0" end="${requestScope.pd.total}">
								 ...
								 <td style="width:5%px;"><a class="goodid">${ggt.GOODID}</a></td>
									...
									<td style="width:5%px"></td><td style="width:1px"></td><td style="width:1px"></td>	
									<td style="width:5%px"></td><td style="width:1px"></td><td style="width:1px"></td>	
								</tr><!--第二行-->
								<tr >	
...
/* 这一块全是待发货的功能  */
	 ...
	 $(document).on('change','#search',function(){
			...
			// $(".tablegood").children.remove();
			 ...
			 $.post('Accountwaitaudit',{flag:"search",user:user,
				 page:page,
				 pagesize:pagesize,
				 blurry:blurry,
				 order:order,
				...
				$(".subdata").attr({"disabled":"disabled"})
					...
					if(this.checked)	
					{
						  var id=$(this).parents("tr").find("td").eq(0).text();
							...
							var type=$("#gdtype").text(); 
							 var pric=$("#goodsprice").text();
							 var price=parseInt(pric);
							 var count=$("#goodscount").text();
							 var remark=$("#goodsremark").text(); 
							 $.post('Accountwaitaudit',{
								 flag:"dele",
								 id:id
							 },function(pd,state){
								 var b = JSON.parse(pd);
			...
			if(yes_no){
				...
				$("input[id='checkone']").each(function(){
					...
					
					{
						/*  获取被选中的id */
					    var id=$(this).parents("tr").find("td").eq(0).text();
						 $.post('Accountwaitaudit',{flag:"dele",
							 goodid:id
							 },function(pd,state){
								 var b = JSON.parse(pd);
								 if(b){
...
function persongood_in_body(GOODS_GOODSTYPE){
   ...
   var  td10="<td style='width:5%px;'><a class='goodid'>"+GOODS_GOODSTYPE.GOODID+"</a></td>";
...
var	td12="<td style='width:45%px;'>商品名:<p id='gdname'>"+GOODS_GOODSTYPE.GOODTITLE+"</p><a hidden='hidden' class='goodid'>"+GOODS_GOODSTYPE.GOODID+"</a><div class='goodcontent' >商品说明:<p id='goodsremark'>"+GOODS_GOODSTYPE.GOODREMARK+"</p></div></td>"	
	...
	var td23="<td><strong>上架时间 :"+GOODS_GOODSTYPE.GOODUP+"</strong></td><td><strong id='downdate'></strong></td>"
