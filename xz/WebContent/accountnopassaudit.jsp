...
<p hidden="hidden" id="upanddown">驳回</p>
		 ...
		 <%-- <table id="example" class="table table-striped table-bordered">
		 	<thead>
		 		<tr>
		 			...
		 			<th>点击量</th>
		 			<th>数量</th>
		 			<th>上架时间 <img id="updown" src="imgbutton/up.jpg"/> </th>
		 			<th>下架时间 <img id="updown" src="imgbutton/up.jpg"/></th>
		 		</tr>
		 		<p>${requestScope.pd}</p>
		 		  <t:if test="${requestScope.pd.data!=null}">
		 		  <t:forEach items="${requestScope.pd.data}" var="ggt" step="1" begin="0" end="${requestScope.pd.total}">
		 		  <tr>
		 		  <th><input type="checkbox" id="checkall"></th>
		 		  <th>${ggt.GOODID}</th>
		 			<th>${ggt.GOODTITLE}</th>
		 			<th>${ggt.GOODTYPE}</th>
		 			<th><strong>${ggt.GOODPRICE}</strong><input type="button" class="alert-info" id="" value="修改" /></th>
		 			<th>${ggt.ORIGINALPRICE}</th>
		 			<th>${ggt.GOODREMARK}</th>
		 			<th>${ggt.GOODCLICK}</th>
		 			<th>${ggt.GOODCOUNT}</th>
		 			<th>${ggt.GOODUP}</th>
		 			<th><strong>${ggt.GOODDOWN}</strong><input type="button" class="alert-info" id="" value="修改" /></th>
		 		  </tr>
		 		  </t:forEach>
		 ...
		 <table id="tablegood" class="table" style="width:100%" >
							<thead>
								...
								<th></th><th></th><th></th><th></th>
         							<th style="width:250px;">
										<!-- <input type="button" class="downgoods alert-info" value="删除" /> -->
										<button class="downgoods alert-info" style="width:100px;height:40px;"><a>删除</a></button>
										<button class='alert-info' style="width:100px;height:40px;"><a id='returnaudit'  role='button' class='btn' data-toggle='modal'>重新上架</a></button>
								</th>
								</tr><!--第一行-->
							</thead>	
							<tbody class="tbody">
							...
							<t:forEach items="${requestScope.pd.data}" var="ggt" begin="0" end="${requestScope.pd.total}">
								...
								<tr >	
									...
									<td>
										<%-- <strong id="downdate">下架时间:${ggt.GOODDOWN}</strong> --%>
									</td>
								</tr>
...
/* 这一块全是待发货的功能  */
	 ...
	 $(document).on('change','#search',function(){
			...
			// $(".tablegood").children.remove();
			 ...
			 $.post('Accountnopassaudit',{flag:"search",user:user,
				 page:page,
				 pagesize:pagesize,
				 blurry:blurry,
				 order:order,
	...
	$(document).on('click','#returnaudit',function(){
		//var id=$(".goodid").text();
			var num=0;
			$("input[id='checkone']").each(function(){
				if(this.checked)	
				...
				$(".subdata").attr({"disabled":"disabled"})
					...
					if(this.checked)	
					{
						  var id=$(this).parents("tr").find("td").eq(0).text();
							...
							// alert(id+","+name+","+type+","+price+","+count+","+remark+",");
							 $.post('Accountnopassaudit',{
								 flag:"dele",
								 id:id
							 },function(pd,state){
								 var b = JSON.parse(pd);
								 if(b){
									 alert("正在跳转");
									 location.href="accountgoodsup.jsp?title="+name+"&type="+type+"&price="+price+"&count="+count+"&remark="+remark;
								 }else{
									 alert("跳转失败，请手动切换");
								 }
			...
			if(yes_no){
				...
				$("input[id='checkone']").each(function(){
					...
					
					{
					    ...
					    var id=$(this).parents("tr").find("td").eq(0).text();
						//alert(id);
						 $.post('Accountnopassaudit',{flag:"dele",
							 id:id
							 },function(pd,state){
								 var b = JSON.parse(pd);
								 if(b){
...
function persongood_in_body(GOODS_GOODSTYPE){
	   ...
	   var  td10="<td style='width:5%px;'><a class='goodid'>"+GOODS_GOODSTYPE.GOODID+"</a></td><td hidden='hidden'>"+GOODS_GOODSTYPE.GOODTITLE+"</td>";
	...
	var	td12="<td style='width:45%px;'>商品名:<p id='gdname'>"+GOODS_GOODSTYPE.GOODTITLE+"</p><a hidden='hidden' class='goodid'>"+GOODS_GOODSTYPE.GOODID+"</a><div class='goodcontent' >商品说明:<p id='goodsremark'>"+GOODS_GOODSTYPE.GOODREMARK+"</p></div></td>"	
			
		var tr1="<tr >"+td10+td11+"<td style='width:5%px;'><a class='goodid'></a></td>"+td12+"</tr>"	
	...
	/* var pp=GOODS_GOODSTYPE.GOODCLICK;
	alert(pp); */
	}
		...
		<div class="modal fade modal11" id="modal-container-12948299" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<input type="text" hidden="hidden"  class="form-control form-group" id="id" value="" />
							...
							<div class="modal-body form-inline">
								<strong class="form-group">下架时间</strong>
								<input type="date" class="form-control form-group downdate"  value="" />
							</div>
							
							<div class="modal-body form-inline">
								<strong class="form-group">说明</strong>
								<textarea name="" rows="3" cols="80" class="form-group" id="text"></textarea>
							</div>
